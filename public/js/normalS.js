...
function buildRoom(cfg) {
  ...
  const screenSurf = add(new THREE.Mesh(new THREE.PlaneGeometry(12.2, 6.8), mats.screenGlow));
  screenSurf.position.set(0, 5, -D / 2 + 0.36);
  ...
  [-1, 1].forEach(side => {
    const strip = add(new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.08, D), mats.accent));
    strip.position.set(side * (W / 2 - 0.05), 1.5, 0);
    const strip2 = strip.clone();
    strip2.position.y = H - 0.5;
    scene.add(strip2);
    roomObjects.push(strip2);
  });
